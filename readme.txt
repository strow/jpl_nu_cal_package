...
==========
  ...
  The I/O is embedded in the main routine, cal_l1c_freqs_and_doppler.m.
  It should be quite easy to pull that into the main program or a
  subroutine.  Since Matlab can write binary files (or hdf or netcdf), I
  would suggest (not knowing anything about a NASA DIS PGE) to just run
  this matlab code from a shell "escape" in the main L1c program before
  writing out the existing L1c file, then read in the new radiances, and
  then write out the final L1c file as you do know with modified
  radiances.  This would minimize the amount of new code that has to be
  written.  But, no idea if that is possible.
